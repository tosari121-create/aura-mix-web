// AURA MIX — Server-side YouTube Data API proxy
// Required Netlify Environment Variable: YOUTUBE_API_KEY
...
exports.handler=async(event)=>{
  ...
  const channelId=action==='old90s' ? 'UCG9gfnA5UDoLvz-tC9B-HKw' : String(body.channelId||'').trim();
  if(action!=='old90s'&&!query)return json(400,{error:'กรุณาระบุคำค้นหา'});
  try{
    const sp=new URLSearchParams({key,part:'snippet',type:'video',maxResults:String(maxResults)});
