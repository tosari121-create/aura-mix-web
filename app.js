(() => {
  ...
  const MIX_SECONDS = 4;
  const AUTO_MIX_OUT_SECONDS = 4;
  const AUTO_MIX_SILENCE_SECONDS = 1;
  const AUTO_MIX_IN_SECONDS = 4;
  const AUTO_MIX_TRANSITION_SECONDS = AUTO_MIX_OUT_SECONDS + AUTO_MIX_SILENCE_SECONDS + AUTO_MIX_IN_SECONDS;
  const TICK_MS = 100;
  ...
  const thumbUrls = track => {
    const rawId = String(track?.id || '').trim();
    const id = rawId.match(/(?:v=|youtu\.be\/|youtube\.com\/(?:embed|shorts|live)\/)?([A-Za-z0-9_-]{6,})/)?.[1] || rawId;
    const custom = String(track?.thumbnail || '').trim();
    if (!id && !custom) return [];
    return [...new Set([
      custom,
      id ? `https://i.ytimg.com/vi/${id}/maxresdefault.jpg` : '',
      id ? `https://i.ytimg.com/vi/${id}/sddefault.jpg` : '',
  ...
  const thumb = track => thumbUrls(track)[0] || '';
  function bindThumbFallback(img, track) {
    const urls = thumbUrls(track);
    let i = 0;
    ...
    img.onload = () => {
      ...
      if (img.classList.contains('cover')) $('coverPlaceholder')?.setAttribute('hidden','');
    };
    img.onerror = () => {
      i += 1;
      ...
      img.hidden = true;
      if (img.classList.contains('cover')) $('coverPlaceholder')?.removeAttribute('hidden');
    };
    img.src = urls[0];
  }
  ...
  function renderSearch(rows) {
    ...
    state.searchResults.forEach((t, i) => {
      ...
      item.innerHTML = `<img class="thumb" alt="ปกเพลง"><span class="item-main"><b>${esc(t.title)}</b><small>${esc(t.uploader || 'YouTube')} • ${fmt(t.duration)}</small></span><span class="item-actions"><span class="circle add">＋</span><span class="circle play">▶</span></span>`;
      ...
      bindThumbFallback(item.querySelector('.thumb'), t);
  ...
  async function artistSearch() {
    ...
    $('searchState').textContent = `กำลังค้นหาเพลงของ ${artist}…`;
    if (!API?.search) { toast('ระบบค้นหา YouTube ไม่พร้อม'); setStatus('SEARCH API ไม่พร้อม', true); return; }
    const requestId = ++state.searchRequestId;
    try {
      // Use the normal YouTube search endpoint directly. This avoids the old
      // artist-specific path that could fail when the API only exposes search.
      const rows = await API.search(`${artist} เพลง`, 'YouTube ทั้งหมด');
      if (requestId !== state.searchRequestId) return;
      renderSearch(rows);
      setStatus(rows.length ? `✅ พบเพลงของ ${artist} ${rows.length} เพลง` : `❌ ไม่พบเพลงของ ${artist}`);
      toast(rows.length ? `✅ พบเพลงของ ${artist} ${rows.length} เพลง` : 'ไม่พบผลลัพธ์');
    } catch (err) {
      if (requestId !== state.searchRequestId) return;
      console.error(err);
      $('searchState').textContent = 'ค้นหาไม่สำเร็จ';
  ...
  function renderQueue() {
    ...
    state.queue.forEach((t, i) => {
      ...
      item.innerHTML = `<span class="queue-number">${i+1}</span><img class="thumb" alt="ปกเพลง"><span class="item-main"><b>${i===state.currentIndex?'▶ ':''}${esc(t.title)}</b><small>${esc(t.uploader||'YouTube')} • ${fmt(t.duration)}</small></span><span class="item-actions"><button class="circle play" type="button">▶</button><button class="circle up" type="button">↑</button><button class="circle down" type="button">↓</button></span>`;
      item.addEventListener('click', e => {
  ...
  function prepareNext() {
    ...
    if (idx >= state.queue.length) {
      ...
      $('nextText').textContent = state.autoMusic ? 'เพลงต่อไป: ✨ รอเพิ่มอัตโนมัติ 1 เพลง' : 'เพลงต่อไป: —';
      return;
    }
    ...
    try {
      safeStop(p); p.setVolume(0);
      // MIX ON: cue the new song at the existing HOOK heuristic so the 60s segment
      // is what rolls into the crossfade. MIX OFF: always cue from the true first second.
      const startSeconds = state.autoMix ? calcHookStart(Number(track.duration) || 180) : 0;
      p.cueVideoById({videoId:String(track.id), startSeconds});
      const started = performance.now();
      const poll = () => {
  ...
  async function prepareAutoMusic(seedTrack = state.current, startImmediatelyIfEmpty = false) {
    if (!state.autoMusic || state.autoPreparing || !API?.search) return null;
    // Add exactly ONE new track only when there is no queued next track.
    if (state.current && state.queue.length > state.currentIndex + 1) return null;
    state.autoPreparing = true;
    if ($('nextText')) $('nextText').textContent = 'เพลงต่อไป: ✨ กำลังเพิ่มเพลงอัตโนมัติ 1 เพลง…';
    try {
      const seedTitle = String(seedTrack?.title || '').replace(/\s+/g, ' ').trim();
      const queries = [
        seedTitle ? `${seedTitle} เพลงไทย` : '',
        'เพลงไทยฮิต 2026 วัยรุ่น',
        'เพลงไทยฮิตปัจจุบัน 2026',
        'SARAN เพลงใหม่ ฮิต',
        'เพลงไทยวัยรุ่น 2026',
        'เพลงไทยฮิต 90s'
      ].filter(Boolean);
      const used = new Set(state.queue.map(x => x.id));
      const blocked = ['รวมเพลง','รวมฮิต','longplay','medley','mix','playlist','compilation','nonstop','karaoke','คาราโอเกะ','live','concert','reaction','shorts','สัมภาษณ์','interview'];
      const pool = [];
      const seen = new Set();
      for (const q of queries) {
        try {
          const rows = await API.search(q, 'YouTube ทั้งหมด');
          for (const row of rows || []) {
            if (!row?.id || used.has(row.id) || seen.has(row.id)) continue;
            seen.add(row.id);
            const title = String(row.title || '').toLowerCase();
            if (blocked.some(k => title.includes(k))) continue;
            if (row.duration && (row.duration < 45 || row.duration > 900)) continue;
            const thai = (String(row.title || '').match(/[ก-๙]/g) || []).length;
            const youth = /(saran|sarun|ฮิต|วัยรุ่น|t-pop|tpop|เพลงไทย|เพลงใหม่|official mv)/i.test(String(row.title || ''));
            const relevance = seedTitle && title.includes(seedTitle.toLowerCase()) ? 4 : 0;
            pool.push({ row, score: thai * 0.35 + (youth ? 3 : 0) + relevance + Math.random() * 1.5 });
          }
        } catch (e) { console.warn('AUTO MUSIC query failed:', q, e); }
      }
      pool.sort((a,b) => b.score - a.score);
      const pick = pool[0]?.row;
      if (!pick) {
        if ($('nextText')) $('nextText').textContent = 'เพลงต่อไป: เพิ่มอัตโนมัติไม่พบเพลงใหม่';
        return null;
      }
      state.queue.push(pick);
      persistQueue();
      renderQueue();
      toast(`✨ เพิ่มอัตโนมัติ 1 เพลง: ${pick.title}`);
      if (startImmediatelyIfEmpty && !state.current) {
        showPage('home');
      ...
      console.error(err);
      if ($('nextText')) $('nextText').textContent = 'เพลงต่อไป: AUTO MUSIC หาไม่สำเร็จ';
      return null;
    } finally {
      state.autoPreparing = false;
    }
  }
  ...
  async function startMix() {
    ...
    $('clockLabel').textContent = `AUTO MIX • CROSSFADE ${AUTO_MIX_TRANSITION_SECONDS}S`;
    ...
    // AUTO MIX transition is intentionally 9 seconds:
    // 4s old track 100→0, 1s complete silence, 4s new track 0→100.
    // The logical track swap happens only after the incoming fade-in finishes,
    // so the next 60-second AUTO MIX window starts from the new track.
    state.mix.phase = 'fade-out';
    setStatus('🎚 AUTO MIX • FADE OUT 4S');

    const masterVolume = master;
    const fadeOutMs = AUTO_MIX_OUT_SECONDS * 1000;
    const silenceMs = AUTO_MIX_SILENCE_SECONDS * 1000;
    const fadeInMs = AUTO_MIX_IN_SECONDS * 1000;
    ...
    const finish = () => {
      ...
      setDeckVolume(incoming, masterVolume);
    ...
    const beginFadeIn = () => {
      if (session !== state.session || !state.mix.running) return;
      state.mix.phase = 'fade-in';
      setStatus('🎚 AUTO MIX • FADE IN 4S');
      setDeckVolume(incoming, 0);
      try { incoming.playVideo(); } catch (_) {}
      ...
      const fadeStep = () => {
        if (session !== state.session || !state.mix.running) return;
        const p = Math.max(0, Math.min(1, (performance.now() - fadeInStart) / fadeInMs));
        setDeckVolume(incoming, p * masterVolume);
    ...
    const beginSilence = () => {
      if (session !== state.session || !state.mix.running) return;
      state.mix.phase = 'silence';
      setStatus('🔇 AUTO MIX • SILENCE 1S');
      setDeckVolume(old, 0);
      setDeckVolume(incoming, 0);
      try { old.stopVideo(); } catch (_) {}
      setTimeout(beginFadeIn, silenceMs);
    };

    const fadeOutStep = () => {
      if (session !== state.session || !state.mix.running) return;
      const p = Math.max(0, Math.min(1, (performance.now() - startedAt) / fadeOutMs));
      setDeckVolume(old, (1 - p) * masterVolume);
      setDeckVolume(incoming, 0);
      if (p >= 1) { beginSilence(); return; }
      requestAnimationFrame(fadeOutStep);
    };

    setDeckVolume(old, masterVolume);
    setDeckVolume(incoming, 0);
    requestAnimationFrame(fadeOutStep);
  }
  ...
  function toggleMix() {
    ...
    updateMode(); syncMenuState();
    // Re-cue the standby deck at the correct start point whenever MIX mode changes.
    // The existing crossfade engine itself is untouched.
    if (state.current && !state.mix.running) {
    ...
    toast(state.autoMix ? 'AUTO MIX เปิด • เพลงใหม่เริ่มช่วง HOOK 60 วิ' : 'AUTO MIX ปิด • เพลงใหม่เริ่มตั้งแต่ต้นจนจบ');
  }
  function toggleAutoMusic() {
    state.autoMusic = !state.autoMusic;
    if (!state.autoMusic) {
      state.autoPreparing = false;
      if (!state.next) $('nextText').textContent = 'เพลงต่อไป: —';
    }
    syncMenuState();
    if (state.autoMusic && state.current) {
      prepareNext();
      const remaining = Math.max(0, Number(state.duration || 0) - Number(state.pos || 0));
      if (state.queue.length <= state.currentIndex + 1 && remaining <= 20) prepareAutoMusic();
    }
    toast(state.autoMusic ? '✨ เพิ่มเพลงอัตโนมัติ: เปิด • ทีละ 1 เพลง' : 'เพิ่มเพลงอัตโนมัติ: ปิด');
  }
  function updateMode() {
    ...
    if ($('modeText')) $('modeText').textContent = state.autoMix ? `AUTO • ${HOOK_SECONDS} วิ / เพลง • CROSSFADE ${AUTO_MIX_TRANSITION_SECONDS} วิ • PRELOAD` : `FULL TRACK • AUTO NEXT • CROSSFADE ${MIX_SECONDS} วิ • PRELOAD`;
  }
  function setCurrentUI(t) {
    $('nowTitle').textContent = t?.title || 'พร้อมเล่น';
    $('nowMeta').textContent = t ? `${t.uploader || 'YouTube'} • ${fmt(t.duration || 180)}` : '—';
    const img = $('coverImg'), ph = $('coverPlaceholder');
    if (t?.id) {
      img.hidden = false;
      ph.hidden = true;
      img.classList.remove('loaded');
      bindThumbFallback(img, t);
    } else {
  ...
  function syncMenuState() {
    $('menuMixState').textContent=state.autoMix?'ON • 60S':'OFF • FULL SONG';
    $('menuAutoState').textContent=state.autoMusic?'ON • ทีละ 1 เพลง':'OFF';
    const b=$('autoMusicBtn');
    if(b){ b.classList.toggle('active', state.autoMusic); const sp=b.querySelector('span'); if(sp) sp.textContent=state.autoMusic?'ON • ทีละ 1 เพลง':'OFF'; }
  }
  ...
  function onYTState(e) {
    ...
    if(e.data===YT.PlayerState.ENDED && !state.mix.running && p===curPlayer()){
      ...
      const finishedId = finishedTrack?.id;
      ...
      if(state.queue.length) {
        startTrack(0);
      } else if(state.autoMusic) {
        // Queue is truly empty: add exactly ONE Thai-leaning recommendation and play it.
        setStatus('✨ AUTO ADD • กำลังเพิ่มเพลง 1 เพลง');
        prepareAutoMusic(finishedTrack, true);
      } else {
        setCurrentUI(null); setStatus('QUEUE EMPTY');
      }
    }
  }
