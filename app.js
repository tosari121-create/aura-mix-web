(() => {
  ...
  function fadeVolume(p, from, to, ms, done) {
    ...
    const frame = now => {
      const q = Math.min(1, (now - started) / Math.max(1, ms));
  ...
  // True dual-deck crossfade.  The Python reference uses two decks and
  // cosine/sine gain curves; here we run those curves at the same time so
  // the old track and the next track actually overlap instead of going
  // silent between them.
  function crossfadeVolumes(oldPlayer, newPlayer, fromVolume, toVolume, ms, done) {
    if (!oldPlayer || !newPlayer) { done?.(false); return; }
    const started = performance.now();
    const duration = Math.max(250, Number(ms) || 4000);
    let finished = false;
    const finish = ok => {
      if (finished) return;
      finished = true;
      try { oldPlayer.setVolume(0); } catch(_) {}
      try { newPlayer.setVolume(Math.max(0, Math.min(100, Math.round(toVolume)))); } catch(_) {}
      done?.(ok);
    };
    const frame = now => {
      if (finished) return;
      const p = Math.min(1, (now - started) / duration);
      // Equal-power style curves: old = cos(theta), new = sin(theta).
      // This is smoother than a linear volume swap and avoids the obvious
      // dip in the middle of the transition.
      const theta = (Math.PI / 2) * p;
      const oldGain = Math.cos(theta);
      const newGain = Math.sin(theta);
      try { oldPlayer.setVolume(Math.max(0, Math.min(100, Math.round(fromVolume * oldGain)))); } catch(_) {}
      try { newPlayer.setVolume(Math.max(0, Math.min(100, Math.round(toVolume * newGain)))); } catch(_) {}
  ...
  async function startMix() {
    ...
    const old = curPlayer(), incoming = standbyPlayer();
    if (!old || !incoming) return;

    const session = state.session;
    const fromVolume = Math.max(0, Math.min(MAX_VOLUME, Number(state.volume) || 0));
    const nextTrackId = state.next.id;

    state.mix.running = true;
    state.mix.phase = 'startNew';
    state.mix.startedAt = performance.now();
    setStatus('🎧 AUTO MIX • PREPARING NEXT DECK');
    $('clockLabel').textContent = 'AUTO MIX • NEXT DECK STARTING';

    try {
      // The standby deck is already CUED. Start it at zero gain first so the
      // audience never hears a hard jump when YouTube transitions into PLAYING.
      incoming.unMute();
    ...
    const playing = await waitForPlayerState(incoming, [YT.PlayerState.PLAYING], 6500);
    if (session !== state.session || !state.next || state.next.id !== nextTrackId) {
      try { incoming.setVolume(0); incoming.pauseVideo(); } catch(_) {}
      return;
    }
    ...
    if (!playing) {
      try {
        old.unMute();
        old.setVolume(fromVolume);
        old.playVideo();
      } catch(_) {}
      state.mix = {running:false, phase:'idle', startedAt:0};
      state.nextReady = false;
      setStatus('⚠️ NEXT DECK START ไม่สำเร็จ • เล่นเพลงเดิมต่อ', true);
      prepareNext();
      return;
    }

    state.mix.phase = 'crossfade';
    setStatus('🎚 AUTO MIX • TRUE CROSSFADE 4S');
    $('clockLabel').textContent = 'AUTO MIX • CROSSFADE 4S';

    crossfadeVolumes(old, incoming, fromVolume, fromVolume, MIX_SECONDS * 1000, ok => {
      if (!ok || session !== state.session) return;
      finishMix(old, incoming);
    });
  }

  function finishMix(oldPlayer = curPlayer(), newPlayer = standbyPlayer()) {
    if (!state.next || !state.mix.running) return;

    const incomingTime = (() => {
      try { return Math.max(0, Number(newPlayer?.getCurrentTime()) || 0); } catch(_) { return 0; }
    })();
    ...
    state.duration = Number(state.current.duration) || 180;
    state.pos = incomingTime;
    state.next = null;
    state.nextIndex = -1;
    state.nextReady = false;
    state.mix = {running:false, phase:'idle', startedAt:0};
    ...
    const elapsed = state.autoMix ? Math.max(0, incomingTime - hook) : incomingTime;
    $('timeLabel').textContent = `${fmt(elapsed)} / ${fmt(visible)}`;
    $('progressBar').style.width = `${Math.max(0, Math.min(1, elapsed / visible)) * 100}%`;

    try { oldPlayer?.setVolume(0); oldPlayer?.stopVideo(); } catch(_) {}
    try { newPlayer?.setVolume(state.volume); } catch(_) {}
  ...
  function monitor() {
    ...
    if (!state.nextReady && elapsed >= Math.max(0, visible - 18)) prepareNext();
    if (state.autoMix) {
  ...
  function drawWave(progress = 0) {
    const c = $('waveCanvas'); if (!c) return; const r = c.getBoundingClientRect(); if (r.width < 10) return;
    const d = Math.max(1, window.devicePixelRatio || 1); c.width = Math.floor(r.width*d); c.height = Math.floor(r.height*d);
  ...
  function onYTState(e) {
    ...
    if(e.data===YT.PlayerState.ENDED && !state.mix.running && p===curPlayer()){
      ...
      if(i<state.queue.length) startTrack(i); else if(state.autoMusic) prepareAutoMusic();
    }
  }
  ...
  function bindUI() {
    ...
    $('playBtn').addEventListener('click',()=>{ if(!state.current){ if(state.selectedSearch)addToQueue(state.selectedSearch,true); else if(state.queue.length)startTrack(state.selectedQueue>=0?state.selectedQueue:0); else {showPage('search');toast('ค้นหาเพลงก่อนครับ');} } else togglePause(); });
  ...
  function boot() {
    ...
    clockId=setInterval(nowClock,1000); timerId=setInterval(monitor,TICK_MS); drawWave(0);
