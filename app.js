(() => {
  ...
  /* ============================================================
     ...
     - MIX timings intentionally stay the same as the supplied Python:
       HOOK 60s / CROSSFADE 9s (4s out + 1s silence + 4s in) / PRELOAD.
     ============================================================ */
  ...
  async function startTrack(index) {
    ...
    state.segmentElapsed = 0; state.lastObservedPos = 0;
    ...
    state.segmentElapsed = 0; state.lastObservedPos = start;
  ...
  async function prepareAutoMusic() {
    ...
    state.autoPreparing = true; $('nextText').textContent = 'เพลงต่อไป: ✨ กำลังเพิ่มเพลงอัตโนมัติ 1 เพลง…';
    try {
      ...
      } else $('nextText').textContent = 'เพลงต่อไป: เพิ่มอัตโนมัติไม่พบเพลงใหม่';
  ...
  async function startMix() {
    ...
    $('clockLabel').textContent = 'AUTO MIX • 9S TRANSITION';
    setStatus('🎚 4S FADE OUT • 1S SILENCE • 4S FADE IN');

    // Keep the next deck CUED and silent. It must NOT be audible during
    // the one-second silence between the two tracks.
    setDeckVolume(incoming, 0);
    try { incoming.pauseVideo(); } catch (_) {}

    // 1) Current song: 100 -> 0 over exactly 4 seconds.
    const fadeOutStart = performance.now();
    await new Promise(resolve => {
      const step = now => {
        if (session !== state.session || !state.mix.running) return resolve();
        const p = Math.max(0, Math.min(1, (now - fadeOutStart) / (FADE_OUT_SECONDS * 1000)));
        setDeckVolume(old, master * (1 - p));
    ...
    setDeckVolume(old, 0);
    try { old.pauseVideo(); } catch (_) {}
    try { old.stopVideo(); } catch (_) {}

    // 2) True silence: both decks at 0 for exactly 1 second.
    state.mix.phase = 'silence';
    setStatus('🔇 SILENCE • 1S');
    setDeckVolume(old, 0);
    setDeckVolume(incoming, 0);
    await sleep(SILENCE_SECONDS * 1000);
    if (session !== state.session || !state.mix.running) return;

    // 3) New song starts only now, from its already-cued position, then
    // ramps 0 -> 100 over exactly 4 seconds.
    state.mix.phase = 'fade-in';
    setStatus('🎵 NEW TRACK • 4S FADE IN');
    setDeckVolume(incoming, 0);
    ...
    await new Promise(resolve => {
      const step = now => {
        if (session !== state.session || !state.mix.running) return resolve();
        const p = Math.max(0, Math.min(1, (now - fadeInStart) / (FADE_IN_SECONDS * 1000)));
        setDeckVolume(incoming, master * p);
        if (p >= 1) return resolve();
        requestAnimationFrame(step);
      };
      requestAnimationFrame(step);
    ...
    // Transition is complete. Only NOW does the next MIX 60-second window
    // start counting, exactly as requested.
    ...
    state.lastObservedPos = state.pos;
    state.next = null;
    state.nextIndex = -1;
    state.nextReady = false;
    state.mix = {running:false, phase:'idle', startedAt:0};
    ...
    renderQueue();
    updateMediaSession();
    $('clockLabel').textContent = state.autoMix ? 'AUTO MIX • COUNTING 60S' : 'AUTO MIX • PLAYING';
    setStatus(state.autoMix ? '✅ NEW TRACK • 60S START' : '✅ AUTO MIX • PLAYING');
    prepareNext();
  }
  ...
  function monitor() {
    ...
    let pct = 0, elapsed = 0, visible = 1;
    if (state.autoMix) {
      // MIX ON: the 60-second clock starts only after the previous 9-second
      // transition has completely finished. The graph follows that same 60s window.
      const delta = Math.max(0, pos - state.lastObservedPos);
      if (delta < 3) state.segmentElapsed += delta;
      state.lastObservedPos = pos;
      elapsed = Math.min(HOOK_SECONDS, state.segmentElapsed);
      visible = HOOK_SECONDS;
      pct = Math.max(0, Math.min(1, elapsed / visible));
    } else {
      // MIX OFF: normal full-song playback from the first second to the end.
      elapsed = Math.max(0, pos);
      visible = Math.max(1, dur);
      pct = Math.max(0, Math.min(1, elapsed / visible));
      state.segmentElapsed = 0;
      state.lastObservedPos = pos;
    }
    ...
    drawWave(pct, performance.now());

    if (!state.nextReady && (state.autoMix ? elapsed >= HOOK_SECONDS - 15 : pos >= dur - MIX_SECONDS - 1)) prepareNext();

    if (state.autoMix) {
      if (state.segmentElapsed >= HOOK_SECONDS - 0.15) {
        if (state.nextReady) startMix();
        else { setStatus('⏳ AUTO MIX • WAITING FOR NEXT', true); prepareNext(); }
      }
    } else {
  ...
  function updateMode() {
    ...
    if ($('modeText')) $('modeText').textContent = state.autoMix ? `AUTO • ${HOOK_SECONDS} วิ / เพลง • CROSSFADE ${MIX_SECONDS} วิ (4↓ + 1เงียบ + 4↑) • PRELOAD` : `FULL TRACK • AUTO NEXT • CROSSFADE ${MIX_SECONDS} วิ (4↓ + 1เงียบ + 4↑) • PRELOAD`;
  }
  ...
  function onYTState(e) {
    ...
    if(e.data===YT.PlayerState.ENDED && !state.mix.running && p===curPlayer()){
      ...
      if(state.queue.length) {
        ...
        // Queue is truly empty now: add only ONE similar song.
        prepareAutoMusic();
