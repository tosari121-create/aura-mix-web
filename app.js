(() => {
  ...
  function startMix() {
    ...
    state.mix.running = true; state.mix.phase = 'fadeOut'; state.mix.startedAt = performance.now();
    setStatus('🎚 AUTO MIX • SMOOTH FADE OUT'); $('clockLabel').textContent = 'AUTO MIX • CROSSFADE 4S';
    const from = state.volume;
    fadeVolume(old, from, 0, MIX_SECONDS*1000, async () => {
      try { old.setVolume(0); old.pauseVideo(); } catch(_) {}
      state.mix.phase = 'startNew';
      try { incoming.unMute(); incoming.setVolume(0); incoming.playVideo(); } catch(_) {}
      const ok = await waitForPlayerState(incoming, [YT.PlayerState.PLAYING], 10000);
      if (!ok) {
        try { old.unMute(); old.setVolume(from); old.playVideo(); } catch(_) {}
        state.mix = {running:false,phase:'idle',startedAt:0}; state.nextReady = false;
        setStatus('✅ เล่นเพลงเดิมต่อ • รอเพลงใหม่', true); prepareNext(); return;
      }
  ...
  function finishMix() {
    const old = curPlayer(), newPlayer = standbyPlayer();
    recordHistory(state.current);
    state.active = state.active === 'a' ? 'b' : 'a';
    state.current = {...state.next}; state.currentIndex = state.nextIndex; state.selectedQueue = state.currentIndex;
    state.duration = Number(state.current.duration) || 180; state.pos = 0;
    state.next = null; state.nextIndex = -1; state.nextReady = false; state.mix = {running:false,phase:'idle',startedAt:0};
    setCurrentUI(state.current); renderQueue(); updateMediaSession(); persistQueue();
    ...
    try { old?.setVolume(0); old?.stopVideo(); } catch(_) {}
